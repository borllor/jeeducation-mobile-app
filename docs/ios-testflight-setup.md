...
| 模式 | 触发条件 | 结果 | 费用 |
|---|---|---|---|
| **未签名编译检查** | 未设置签名 Secrets | 仅验证 iOS 工程能否编译通过 | 免费额度内 |
| **签名 + TestFlight** | 设置了 4 个签名 Secrets | 打出 `.ipa` 并上传到 TestFlight | 需 $99/年 Apple 账号 |

> **费用提醒**：macOS runner 按 **10 倍**消耗免费额度（私有仓库每月 2000 分钟 → 约 200 macOS 分钟）。一次 iOS 构建约 10～20 分钟，所以工作流**只在手动触发或打 `v*` tag 时运行**，不会每次 push 都跑。
